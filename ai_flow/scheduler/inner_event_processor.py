...
from ai_flow.model.internal.events import InnerEventType, EventContextConstant
...
class InnerEventProcessor(object):
    ...
    def process(self, event: Event) -> Union[WorkflowExecutionScheduleCommand,
        ...
        inner_event_type = InnerEventType(event.event_key.name)
        if InnerEventType.START_WORKFLOW_EXECUTION == inner_event_type:
        ...
        elif InnerEventType.STOP_WORKFLOW_EXECUTION == inner_event_type:
            context = json.loads(event.context)
        ...
        elif InnerEventType.START_TASK_EXECUTION == inner_event_type:
            context = json.loads(event.context)
            workflow_execution_id = context[EventContextConstant.WORKFLOW_EXECUTION_ID]
        ...
        elif InnerEventType.RESTART_TASK_EXECUTION == inner_event_type:
            return self.restart_task_execution(event)
        elif InnerEventType.STOP_TASK_EXECUTION == inner_event_type:
            context = json.loads(event.context)
        ...
        elif InnerEventType.PERIODIC_RUN_WORKFLOW == inner_event_type:
            context = json.loads(event.context)
        ...
        elif InnerEventType.PERIODIC_RUN_TASK == inner_event_type:
            return self.restart_task_execution(event)
        else:
            pass
